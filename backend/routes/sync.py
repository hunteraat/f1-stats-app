...
async def fetch_data_async(session, endpoint, params, max_retries=3, initial_delay=1):
    """Fetch data asynchronously using aiohttp with retries for rate limiting"""
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                ...
                if status == 200:
                    ...
                    logger.info(f"Received {len(data) if data else 0} records for {endpoint}")
                    if logger.isEnabledFor(logging.DEBUG):
                        if data and len(data) > 0:
                            logger.debug(f"First few records: {json.dumps(data[:3], indent=2)}")
                        else:
                            logger.debug(f"No records in response: {json.dumps(data, indent=2)}")
                    return data
                elif status == 429:  # Rate limited
                    if retry_count < max_retries:
                        logger.warning(f"Rate limited on {endpoint}. Retrying in {delay} seconds...")
                        await asyncio.sleep(delay)
                        delay *= 2  # Exponential backoff
                        retry_count += 1
                        continue
                    else:
                        logger.error(f"Max retries reached for {endpoint} after rate limit")
                        return None
                else:
                    logger.error(f"Error response for {endpoint}: Status {status}")
                    try:
            ...
            logger.error(f"Exception during {endpoint} request: {str(e)}")
            if retry_count < max_retries:
...
async def fetch_data_by_month(session, endpoint, start_date, end_date, param_name='date'):
    ...
    while current_date < final_date:
        ...
        logger.info(f"Fetching {endpoint} data from {current_date_str} to {next_date_str}")
        params = {
            f'{param_name}>': current_date_str,
            f'{param_name}<': next_date_str
        }
...
async def sync_f1_data(year):
    # Initialize or get year data outside try block
    ...
    try:
        ...
        logger.info(f"Starting optimized F1 data sync for {year}...")
        ...
        if existing_sessions:
            logger.info(f"Found {len(existing_sessions)} existing sessions in database for {year}")
            sessions_data = [{
            ...
            existing_drivers = Driver.query.join(DriverSession).join(Session).filter(Session.year == year).all()
            logger.info(f"Found {len(existing_drivers)} existing drivers in database for {year}")
            all_drivers_data = [{
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    for sess in sessions_data:
                        cache_entry = SessionKeyCache(
                            ...
                            date_start=datetime.fromisoformat(sess['date_start'].replace('Z', '+00:00')) if sess.get('date_start') else None,
                            location=sess.get('location')
                        )
                        existing = SessionKeyCache.query.filter_by(year=year, session_key=sess['session_key']).first()
                        if existing:
            ...
            async with aiohttp.ClientSession() as session:
                for session_key in session_keys:
                    drivers = await fetch_data_async(session, "drivers", {'session_key': session_key})
                    if drivers:
        ...
        update_sync_progress(year_data, 20, "Checking and fetching position and lap data...")
        
        # Check for existing position and lap data
        existing_positions = Position.query.join(DriverSession).join(Session).filter(Session.year == year).first()
        existing_laps = Lap.query.join(DriverSession).join(Session).filter(Session.year == year).first()
        ...
        async with aiohttp.ClientSession() as session:
            ...
            start_date = None
            if is_current_year and year_data.last_incremental_sync and existing_positions and existing_laps:
            ...
            end_date = (datetime.now(timezone.utc).isoformat() if is_current_year 
                       else datetime(year, 12, 31, 23, 59, 59, 999999, tzinfo=timezone.utc).isoformat())

            logger.info(f"Checking data status for {year}:")
            if not existing_positions:
                logger.info("No existing position data found, fetching...")
                positions_data = await fetch_data_by_month(session, "position", start_date, end_date, "date")
            else:
            ...
            if not existing_laps:
                ...
                laps_data = await fetch_data_by_month(session, "laps", start_date, end_date, "date_start")
            else:
                logger.info("Found existing lap data, skipping fetch")

            # Only consider it a failure if we tried to fetch data and failed
            if (not existing_positions and not positions_data) or (not existing_laps and not laps_data):
                logger.error("Failed to fetch required position or lap data. You can retry the sync to attempt fetching this data again.")
                ...
                return jsonify({
                    ...
                    'can_retry': True,
                    'missing_position_data': not existing_positions and not positions_data,
                    'missing_lap_data': not existing_laps and not laps_data
                }), 500

            logger.info(f"Data status for {year}:")
            logger.info(f"- Positions: {'Using existing data' if existing_positions else f'Fetched {len(positions_data) if positions_data else 0} new records'}")
            logger.info(f"- Laps: {'Using existing data' if existing_laps else f'Fetched {len(laps_data) if laps_data else 0} new records'}")
        ...
        if positions_data:
                ...
                for pos in batch:
                    try:
                        ...
                        if not session_key or not driver_number:
                            logger.warning(f"Skipping position record - missing key data: session_key={session_key}, driver_number={driver_number}")
                            skipped_positions += 1
                            continue
                        
                        # Get or create driver session
                        driver_session = db.session.query(DriverSession).join(
                            Session, DriverSession.session_id == Session.id
                        ).join(
                            Driver, DriverSession.driver_id == Driver.id
                        ).filter(
                        ...
                        if not driver_session:
                            ...
                            if not driver:
                                logger.warning(f"Driver not found for driver_number: {driver_number}")
                                skipped_positions += 1
                                continue
                        ...
                        try:
                            position_date = make_aware(datetime.fromisoformat(pos['date'].replace('Z', '+00:00')))
                            position = Position(
            ...
            logger.info(f"Position processing complete. Processed: {positions_processed}, Skipped: {skipped_positions}")
            
            if skipped_positions > 0:
                logger.warning(f"Skipped {skipped_positions} position records due to errors")

        if laps_data:
            update_sync_progress(year_data, 80, "Processing new lap data...")
            # Process lap times
            # Clear existing laps for the affected period if we're updating
            if start_date and is_current_year:
                deleted_count = Lap.query.filter(
                    Lap.driver_session_id.in_(
                        db.session.query(DriverSession.id).join(Session).filter(
                            Session.date_start >= make_aware(datetime.fromisoformat(start_date))
                        )
                    )
                ).delete(synchronize_session='fetch')
                logger.info(f"Deleted {deleted_count} existing laps for incremental update")
        ...
        # Update year data
        year_data.drivers_count = len(all_drivers_data) if all_drivers_data else Driver.query.join(DriverSession).join(Session).filter(Session.year == year).count()
        year_data.sessions_count = len(sessions_data) if sessions_data else Session.query.filter_by(year=year).count()
...
def get_sync_status(year):
    ...
    return jsonify({
        ...
        'last_synced': year_data.last_synced.isoformat() if year_data.last_synced else None,
        'last_incremental': year_data.last_incremental_sync.isoformat() if year_data.last_incremental_sync else None
    })
...
def clear_lap_data(year):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Failed to clear lap data: {str(e)}'
        }), 500 